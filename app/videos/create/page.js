...
function VideoCreateContent() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const pollVideoStatus = async (requestId, statusUrl, model, onProgress) => {
        ...
        for (let i = 0; i < maxAttempts; i++) {
          ...
          const res = await fetch('/api/video/status', {
            ...
            body: JSON.stringify({ requestId, statusUrl, model }),
          })
      ...
      if (videoData.status === 'queued' && videoData.requestId) {
        setProgress(50)
        finalVideoUrl = await pollVideoStatus(videoData.requestId, videoData.statusUrl, videoData.model, (p) => setProgress(p))
      }
